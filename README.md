...
- **单条重定位项的字段拆解**：`r_offset` / `r_info` / `r_addend` 三个字段按字节着色分组，
  ...
  同时给出各自的二进制、十六进制与十进制，并写出还原算式 `r_info = (符号索引 << 32) | 类型`。
- **被修补的位置（改的是哪条指令、哪几位）**：显示 `r_offset` → **段内偏移** → 文件偏移 → 所在段，
  然后**用高亮色块标出被修改的那几条指令**，并逐条标出**被改动的位域**：
  `R_RISCV_CALL_PLT` 会高亮 `auipc`（改 `imm[31:12]` 高 20 位）与紧跟的 `jalr`（改 `imm[11:0]` 低 12 位）两条，
  之后的一条仅作上下文并弱化显示；`R_RISCV_HI20/LO12_I/LO12_S/JAL/BRANCH` 等类型各自对应不同的指令条数与位域。
  数据段重定位（`R_RISCV_RELATIVE/64/32/JUMP_SLOT` 等）不会硬解成指令，而是显示「将被写入的那几个字节」。
